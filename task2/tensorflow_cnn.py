...
class CNN(Model):
    def __init__(self,
                 epochs=15,
                 use_own_factors=False,
                 n_factors=158,
                 loss=losses.MeanSquaredError()):
        super(CNN, self).__init__()
        self.cnn_model = None
        self.use_own_factors = use_own_factors
        self.n_factors = n_factors
        self.epochs = epochs
        self.loss = loss

    def fit(self, dataset: Dataset, own_dtrain=None, own_dvalid=None):
        if self.use_own_factors:
            dtrain, dvalid = own_dtrain, own_dvalid
        else:
    ...
    def predict(self, dataset: DatasetH, segment: Union[Text, slice] = "test", own_test=None) -> object:
        if self.use_own_factors:
            x_test, y_test = own_test['feature'], own_test['label']
        else:
